/**
 * Xiishopy ERP - Firestore Seed Runner
 * Lives inside functions/ so firebase-admin resolves correctly.
...
async function seed() {
  ...
  // 3. Seed Products collection (use Firestore Timestamps for date fields)
  ...
  // 5. Seed sample orders with fields matching OrderModel expectations
  const orders = [
    {
      id: 'order-001',
  ...
  // 6. Seed sample payments matching PaymentModel expectations
  const payments = [
    {
      id: 'payment-001',
      orderId: 'order-001',
  ...
  // 7. Seed sample shipments matching ShipmentModel expectations
  const shipments = [
    {
      id: 'shipment-001',
      orderId: 'order-001',
